...
ONE-CLICK CLEAN EXTRACTION BUILD FOR DBEAVER

Run this complete file with Alt+X.
Do not use Ctrl+Enter for this file.

This resets only the generated schema:
    pms_extract_2010_2023

It does not delete or modify public source tables.
*/

DROP SCHEMA IF EXISTS "pms_extract_2010_2023" CASCADE;
CREATE SCHEMA "pms_extract_2010_2023";


/*
Legacy PMS tables contain a mixture of DATE, TIMESTAMP and VARCHAR date fields.
This function converts supported values to DATE and returns NULL for blank or
unparseable values, preventing VARCHAR-vs-DATE COALESCE errors.
*/
CREATE OR REPLACE FUNCTION "pms_extract_2010_2023"."safe_date"(p_value text)
RETURNS date
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    IF v IS NULL
       OR LOWER(v) IN ('null', 'none', 'n/a', 'na', 'nil', '-', '--') THEN
        RETURN NULL;
    END IF;

    -- ISO date or timestamp: 2023-07-31 / 2023-07-31 10:30:00
    ...
    -- Last controlled PostgreSQL attempt.
    RETURN v::date;
...
-- DBeaver Auto-commit edition:
-- Explicit BEGIN/COMMIT were intentionally removed.
-- Run the entire file with Execute SQL Script (Alt+X).
-- In DBeaver preferences, enable 'Stop script execution on error'.
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"("bill_date"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("receiveddate"::text)) >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"("bill_date"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("receiveddate"::text)) < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"("bill_date"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)) >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"("bill_date"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)) < c."end_date_exclusive";
...
SELECT
    ...
    s."nature_of_tax",
    s."tax_percentage",
    "pms_extract_2010_2023"."safe_date"(s."tax_period_from"::text) AS "tax_period_from",
    "pms_extract_2010_2023"."safe_date"(s."tax_period_to"::text) AS "tax_period_to",
    s."final_amount",
    s."bill_amount",
    s."status",
    s."billcharge",
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"("billdate"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)) >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"("billdate"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)) < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE "pms_extract_2010_2023"."safe_date"("billdate"::text) >= c."start_date" AND "pms_extract_2010_2023"."safe_date"("billdate"::text) < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"("payment_date"::text), "pms_extract_2010_2023"."safe_date"("transaction_date"::text)) >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"("payment_date"::text), "pms_extract_2010_2023"."safe_date"("transaction_date"::text)) < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"("receiveddate"::text), "pms_extract_2010_2023"."safe_date"("billdate"::text), "pms_extract_2010_2023"."safe_date"("createddate"::text)) >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"("receiveddate"::text), "pms_extract_2010_2023"."safe_date"("billdate"::text), "pms_extract_2010_2023"."safe_date"("createddate"::text)) < c."end_date_exclusive";
...
DROP TABLE IF EXISTS "pms_extract_2010_2023"."date_parse_issues";
CREATE TABLE "pms_extract_2010_2023"."date_parse_issues" AS
SELECT
    'm_taxes'::text AS source_table,
    s."tax_id"::text AS source_id,
    'valid_from'::text AS source_column,
    s."valid_from"::text AS raw_value
FROM "public"."m_taxes" AS s
WHERE NULLIF(BTRIM(s."valid_from"::text), '') IS NOT NULL
  AND "pms_extract_2010_2023"."safe_date"(s."valid_from"::text) IS NULL
...
SELECT
    ...
    s."valid_upto"::text
FROM "public"."m_taxes" AS s
WHERE NULLIF(BTRIM(s."valid_upto"::text), '') IS NOT NULL
  AND "pms_extract_2010_2023"."safe_date"(s."valid_upto"::text) IS NULL

UNION ALL

SELECT
    'm_taxes_history',
    s."tax_revision_id"::text,
    'valid_from',
    s."valid_from"::text
...
SELECT
    ...
    s."tax_percent",
    s."fixed_tax_amount",
    "pms_extract_2010_2023"."safe_date"(s."valid_from"::text) AS "valid_from",
    "pms_extract_2010_2023"."safe_date"(s."valid_upto"::text) AS "valid_upto",
    s."is_active",
    s."tax_type_id",
    s."tax_rev_id",
    s."gl_code",
...
FROM "public"."m_taxes" AS s
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."valid_upto"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."valid_from"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."tax_period_to"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."tax_period_from"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."to_date"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."from_date"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."period_to"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."period_from"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."commencement_end_date"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."commencement_date"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."rate_applicable_upto"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."rate_applicable_from"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
CROSS JOIN "pms_extract_2010_2023"."extract_config" AS c
WHERE COALESCE("pms_extract_2010_2023"."safe_date"(s."effective_to"::text), DATE '9999-12-31') >= c."start_date" AND COALESCE("pms_extract_2010_2023"."safe_date"(s."effective_from"::text), DATE '0001-01-01') < c."end_date_exclusive";
...
SELECT
    ...
    date_trunc('month', COALESCE("pms_extract_2010_2023"."safe_date"("bill_date"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)))::date AS "month_start",
...
SELECT
    ...
    date_trunc('month', COALESCE("pms_extract_2010_2023"."safe_date"("billdate"::text), "pms_extract_2010_2023"."safe_date"("bill_creation_date"::text), "pms_extract_2010_2023"."safe_date"("periodfrom"::text)))::date AS "month_start",
...
SELECT
    ...
    date_trunc('month', COALESCE("pms_extract_2010_2023"."safe_date"("payment_date"::text), "pms_extract_2010_2023"."safe_date"("transaction_date"::text)))::date AS "month_start",
...
SELECT
    ...
    date_trunc('month', COALESCE("pms_extract_2010_2023"."safe_date"("receiveddate"::text), "pms_extract_2010_2023"."safe_date"("billdate"::text)))::date AS "month_start",
...
SELECT
    ...
    "pms_extract_2010_2023"."safe_date"("from_date"::text) AS "observation_date",
...
WITH expected(table_name) AS (
    VALUES
        ...
        ('dim_verified_tenancy_snapshot'),
        ('fact_revenue_current'),
        ('fact_revenue_legacy'),
        ('fact_bills_simple'),
        ('fact_cash_monthly_bills'),
        ('fact_monthly_bills'),
        ('fact_legacy_general_bills'),
        ('fact_monthly_bill_heads'),
        ('fact_monthly_bill_taxes'),
        ('fact_payment_current'),
        ('fact_payment_legacy'),
        ('rule_tax_master'),
        ('rule_tax_history'),
        ('rule_tax_period'),
        ('rule_property_tax_rates'),
        ('bridge_tenant_tax'),
        ('bridge_applicant_tax'),
        ('fact_rent_slab'),
        ('fact_rent_slab_sor'),
        ('fact_additional_rent'),
        ('fact_additional_rent_slab'),
        ('fact_liability_rent'),
        ('fact_plot_fair_market_value'),
        ('fact_plot_rr_land_value'),
        ('fact_plot_sor_market_value'),
        ('rule_zone_rate'),
        ('rule_generic_rate'),
        ('fact_inspection'),
        ('fact_inspection_encroachment'),
        ('fact_inspection_construction'),
        ('fact_inspection_change_of_use'),
        ('fact_inspection_sublet'),
        ('fact_inspection_unauthorized_repairs'),
        ('fact_breach'),
        ('fact_mrtp_notice'),
        ('fact_legal_notice'),
        ('fact_legal_suit'),
        ('bridge_legal_suit_plot'),
        ('bridge_legal_suit_tenancy'),
        ('model_revenue_monthly_by_source'),
        ('model_billing_monthly_by_source'),
        ('model_payment_monthly_by_source'),
        ('model_land_value_observations'),
...
SELECT
    (SELECT COUNT(*) FROM expected) AS expected_table_count,
    (SELECT COUNT(*) FROM actual) AS actual_table_count,
    (SELECT COUNT(*)
       FROM expected e
       LEFT JOIN actual a USING (table_name)
      WHERE a.table_name IS NULL) AS missing_table_count,
    CASE
        WHEN NOT EXISTS (
            SELECT 1
            FROM expected e
...
SELECT
    ...
    end_date_exclusive,
    created_at
FROM "pms_extract_2010_2023"."extract_config";

SELECT
    'dim_division' AS table_name, COUNT(*)::bigint AS row_count
FROM "pms_extract_2010_2023"."dim_division"
UNION ALL
SELECT 'dim_unit', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."dim_unit"
UNION ALL
SELECT 'dim_estate', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."dim_estate"
UNION ALL
SELECT 'dim_plot', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."dim_plot"
UNION ALL
SELECT 'fact_revenue_current', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_revenue_current"
UNION ALL
SELECT 'fact_revenue_legacy', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_revenue_legacy"
UNION ALL
SELECT 'fact_monthly_bills', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_monthly_bills"
UNION ALL
SELECT 'fact_payment_current', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_payment_current"
UNION ALL
SELECT 'fact_plot_fair_market_value', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_plot_fair_market_value"
UNION ALL
SELECT 'fact_inspection', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_inspection"
UNION ALL
SELECT 'fact_legal_suit', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."fact_legal_suit"
UNION ALL
SELECT 'embedding_source_text', COUNT(*)::bigint
FROM "pms_extract_2010_2023"."embedding_source_text"
ORDER BY table_name;


/* Date parsing audit: zero rows is ideal; nonzero rows require data review. */
SELECT
    COUNT(*) AS invalid_critical_date_count
FROM "pms_extract_2010_2023"."date_parse_issues";
